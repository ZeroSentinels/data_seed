...
-- Idempotencia: los nombres que esta misma migracion crea, para que un
-- segundo run no aborte con "policy already exists".
...
drop policy if exists "agents_select_members" on public.agents;
...
drop policy if exists "connectors_select_members" on public.connectors;
